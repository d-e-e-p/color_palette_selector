...
//
// TODO: make into module and use 
//  import "constants";
//

function random_bg_color() {
    // a random grey color with r,g,b offset by 'offset/2' from middle
    var offset = 20;
    var x = Math.random() * (256 - offset);
    var r = Math.floor(x + (Math.random() * offset));
    var g = Math.floor(x + (Math.random() * offset));
    var b = Math.floor(x + (Math.random() * offset));
    return [r,g,b];
}
...
const GA_ID = document.documentElement.getAttribute("ga-id");
    ...
    /*
        let worker = new Worker('/js/colorpicker/worker.js');    
        worker.onmessage = function(e) 
        {
            display_results(e.data);
        };

        let worker_hsluv = new Worker('/js/colorpicker/workerhsluv.js');    
     ...
     */
        ...
        let worker_okhsl = new Worker('/js/colorpicker/workerokhsl.js');    
        /*
        let worker_okhsl = new Worker( 
            new URL('workerokhsl.js', import.meta.url),
            {type: 'module'}
         );
        */
        ...
        function update(async=true) 
        {
            ...
            function update_hsl_inputs(prefix, to_hsl)
            {
                let hsl = to_hsl(r,g,b);
                let hsl_a = 0.5 + 0.5*hsl[1]*Math.cos(hsl[0]*2*Math.PI);
                let hsl_b = 0.5 + 0.5*hsl[1]*Math.sin(hsl[0]*2*Math.PI);
            ...
            function update_rgb_inputs(rgb)
            {
                var [r,b,g] = rgb
            ...
            */
            ...
            function callback_on_bg_change(rgbhex, lightness) {
                ...
                */
                ...
                document.body.style.backgroundColor = rgbhex;

                // update mesh slider to mark location of bg on lightness scale
        ...
        {   
            ...
            function setup_rgb_handlers_input([r,g,b])
            {
                setup_input_handler(document.getElementById('rgb_r_input'), function(r_)
                {
                    r = r_;
                });
                setup_input_handler(document.getElementById('rgb_g_input'), function(g_)
                {
                    g = g_;
                });
                setup_input_handler(document.getElementById('rgb_b_input'), function(b_)
                {
                    b = b_;
                });
            }

            function setup_hsl_handlers_input(prefix, to_hsl, from_hsl)
            {
            ...
            function setup_hsl_handlers_canvas(prefix, to_hsl, from_hsl)
            {
            ...
            */
            ...
            //setup_hsl_handlers_canvas("hsl", rgb_to_hsl, hsl_to_rgb);
